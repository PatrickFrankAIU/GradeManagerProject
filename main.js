...
function displayResults() {
    ...
    if (gradeBook.length === 0) {
        ...
        html += '<table id="gradeTable">';
            html += '<thead><tr><th>Name</th><th>Class</th><th>Assignment</th><th>Grade</th><th>Average</th><th>Comments</th></tr></thead>';
            html += '<tbody>';
            gradeBook.forEach(student => {
                html += `<tr><td>${student.studentName}</td><td>${student.className}</td><td>${student.assignmentType}</td><td>${student.grade}</td><td>***</td><td>${student.comments}</td></tr>`;
            });
            html += '</tbody></table>';
    }
